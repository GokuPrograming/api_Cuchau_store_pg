...
let clientId =
  "ATsXThlRKQMIDRsC0xX-EWt57Vg_FkznXcQNTrWdHgT-X2337ZiEuWGnnOgtubRXGfMJICcIOZ_lZ6aY";
let clientSecret =
  "EPBsfImy1LBYQvyB02m-IsTvBIoZCudBOcRWGQhU2WkulbW1FwL6jJ0mpWEPbC7167A58-WFs42OXyHX";
...
const storage = multer.diskStorage({
    destination: (req, file, cb) => {
      cb(null, "img_productos/"); // Asegúrate de que esta carpeta exista
    },
...
router.get("/estados/:id", async (req, res) => {
  ...
  try {
    ...
    const result = await client.query(
      "SELECT s.id, s.name FROM states s JOIN countries c ON s.id_country = c.id WHERE c.id = $1",
      [id]
    );
    const results = { data: result ? result.rows : null };
    res.json(results);
...
router.post("/login", async (req, res) => {
  ...
  try {
    ...
    const result = await client.query(
      "SELECT id_usuario,id_rol,correo,password FROM usuario WHERE correo = $1 AND password = $2",
...
const validarToken = (req, res) => {
  try {
    ...
    if (
      !decodedToken ||
      Number(decodedToken.id_usuario) !== Number(id_usuario)
    ) {
      throw new Error("Token inválido");
    }
...
router.post("/addToCar", async (req, res) => {
  ...
  try {
    ...
    // Obtener el stock del producto
    const almacen_row = await client.query(
    ...
    await client.query(
      "update producto set almacen=almacen-$1 where id_producto=$2",
...
router.post("/realizar_compra", async (req, res) => {
  ...
  try {
    ...
    const clienteID = await client.query(
    ...
    if (codigo !== null && codigo !== undefined) {
      const buscarCupon = await client.query(
        "SELECT id_cupon_d, porcentaje_descuento, codigo FROM cupon_disponible WHERE codigo = $1",
        [codigo]
      );
...
router.post("/user/MostrarPedido", async (req, res) => {
  ...
  try {
    const client = await db.connect();
    const result = await client.query(
      "select id_pedido,fecha from pedido where id_usuario =$1 order by fecha desc;",
      [id_usuario]
    );
    const results = { data: result ? result.rows : null };
    res.json(results);
...
router.post("/user/MostrarDatosDelPedido/", async (req, res) => {
  ...
  try {
    const client = await db.connect();
    const result = await client.query(
      "SELECT p2.producto, SUM(cantidad) AS total_cantidad, SUM(subtotal) AS total_subtotal,p.id_pedido FROM venta_detalle JOIN pedido p ON venta_detalle.id_pedido = p.id_pedido JOIN producto p2 ON venta_detalle.id_producto = p2.id_producto WHERE p.id_usuario =$1 AND p.id_pedido = $2 GROUP BY p2.producto, p.id_pedido",
  ...
  try {
    ...
    const results = {
      data: {
        ...
        total_cuentas_ultimo_mes:
          resultTotalCuentasUltimoMes.rows[0].total_cuentas_ultimo_mes,
        total_ventas: resultTotalVentas.rows[0].total_ventas,
        total_ventas_ultimo_mes:
          resultTotalVentasUltimoMes.rows[0].total_ventas_ultimo_mes,
      },
    };
    res.json(results);
  } catch (err) {
...
router.get("/producto/:id", async (req, res) => {
  ...
  try {
    ...
    const result = await client.query(
      `
            SELECT producto.id_categoria, id_producto, producto, precio, almacen, imagen, producto.descripcion, categoria, proveedor, p.id_proveedor
            FROM producto
    ...
    if (results) {
      ...
      results.imagen_url = `${req.protocol}://${req.get(
        "host"
      )}/img_productos/${results.imagen}`;
    }
...
router.get("/proveedor", async (req, res) => {
  ...
  try {
    client = await db.connect();
    const result = await client.query(
      `select proveedor.proveedor as proveedor, id_proveedor  from proveedor`
    );
    const results = { data: result ? result.rows : null };
    res.json(results);
  } catch (error) {
...
router.post("/registrarProductos", async (req, res) => {
  ...
  try {
    const client = await db.connect();
    const result = await client.query(
      "UPDATE producto SET producto = $1, precio = $2, almacen = $3, id_proveedor = $4, id_categoria = $5, descripcion = $6, precio=$8 WHERE id_producto = $7",
      [
        producto,
        precio,
        almacen,
...
router.get("/product", async (req, res) => {
  ...
  try {
    ...
    // Recorrer cada objeto y agregar la URL completa de la imagen
    const resultsWithImageUrls = results.map((product) => ({
      ...product,
      imagen_url: `${req.protocol}://${req.get("host")}/img_productos/${
        product.imagen
      }`,
    }));

    console.log(resultsWithImageUrls);
    res.json({ data: resultsWithImageUrls });
  } catch (error) {
    console.error("Error al obtener el producto:", error);
    res.status(500).json({ message: "Error interno del servidor" });
  } finally {
